...
class TestConvolutionNNDeviceType(NNTestCase):
    ...
    def test_conv2d_deterministic_mode(self, device, dtype):
        # intel/torch-xpu-ops#3216: when deterministic algorithms are
        # requested, the oneDNN convolution primitive must select a
        # deterministic implementation and produce reproducible results
        ...
        with DeterministicGuard(True):
            ...
            for _ in range(10):
                self.assertEqual(
                    first, F.conv2d(x, w, padding=1), atol=0.0, rtol=0.0
                )
