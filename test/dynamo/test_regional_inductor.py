...
def forward(self, arg0_1, arg1_1):
    ...
    def test_flex_attention(self, serialize):
        ...
        with _testing_capture_invoke_subgraph_inductor_compile_gms() as captured_gms:
            ...
            self.assertEqual(len(captured_gms), 2)
            # inductor compiled forward graph
            fwd_code = captured_gms[0].code.strip()
            # On ROCm, auto-detection sets BLOCKS_ARE_CONTIGUOUS=True for
            # contiguous masks. Normalize for graph comparison.
            fwd_code = fwd_code.replace(
...
def forward(self, primals_0, primals_1, primals_2, primals_3, primals_4, primals_5, primals_6, primals_7, primals_8):
    ...
    return (getitem, primals_0, primals_1, primals_2, primals_3, primals_4, primals_5, primals_6, primals_7, primals_8, alias_2, alias_3)""",
            ...
            # inductor compiled backward graph
            bwd_code = captured_gms[1].code.strip()
            bwd_code = bwd_code.replace(
                "'BLOCKS_ARE_CONTIGUOUS': True",
                "'BLOCKS_ARE_CONTIGUOUS': False",
            )
            self.assertExpectedInline(
