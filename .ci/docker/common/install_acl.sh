...
SCONS_COMPILER_CACHE_ARGS=()
if [[ "${USE_CCACHE}" != "0" ]]; then
  echo "Using ccache for ACL build"
  command -v ccache >/dev/null
  SCONS_COMPILER_CACHE_ARGS+=(compiler_cache=ccache)
fi
...
(
  ...
  # Build with scons
  scons -j${MAX_JOBS} "${SCONS_COMPILER_CACHE_ARGS[@]}" \
    Werror=0 debug=0 neon=1 opencl=0 embed_kernels=0 \
